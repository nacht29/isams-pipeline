...
TOKEN_URL = 'https://school_id.isamshosting.cloud/auth/connect/token'
API_BASE_URL = 'https://school_id.isamshosting.cloud'
...
def multi_page_endpoint(access_token:str, full_api_path:str, endpoint:str, endpoint_info:dict, trunc_flag:bool):
	...
	page_size = 1000
	while True:
		print(Fore.YELLOW + f'Processing {(cur_page * page_size) if (cur_page * page_size) < totalCount else totalCount} out of {totalCount}')
		...
		# read API response
		# endpoint_onject is the data to be extracted from API (lis of dict)
		try:
		...
		print(Fore.CYAN + f'Processed {(cur_page * page_size) if (cur_page * page_size) < totalCount else totalCount} out of {totalCount} for {endpoint}')

		# break loop if curren iteration already covers all rows
...
def main():
	...
	for endpoint, endpoint_info in isams_dataset_endpoints.items():
		...
		# if endpoint not in ['billing_cycles']:
		# 	continue
